  ...
  const loadAnalytics = async () => {
    try {
      const [summaryRes, locationRes, categoryRes, usedRes, costRes] = await Promise.all([
        ...
        axios.get('/api/analytics/most-used'),
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-slate-400 text-sm mt-1">Equipment inventory and usage analytics</p>
      </div>
      ...
      {summary && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="card p-6 bg-gradient-to-br from-red-900/20 to-red-800/20 border-red-600">
            <p className="text-slate-400 text-sm font-medium">⚠️ Low Stock</p>
      ...
      <div className="card p-6">
        ...
        {byLocation.length === 0 ? (
          <p className="text-slate-400 text-center py-8">No location data available</p>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {byLocation.map((loc, idx) => (
              <div key={idx} className="bg-slate-700/50 rounded-lg p-4 border border-slate-600 hover:border-slate-500 transition-colors">
                ...
                <p className="text-sm text-slate-400 mt-1">📦 {loc.item_count} type(s)</p>
              </div>
            ))}
          </div>
        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {byCategory.map((cat, idx) => (
              <div key={idx} className="bg-slate-700/50 rounded-lg p-4 border border-slate-600 hover:border-slate-500 transition-colors">
                ...
                <p className="text-sm text-slate-400 mt-1">📦 {cat.item_count} item(s)</p>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="card p-6">
        ...
        {mostUsed.length === 0 ? (
          <p className="text-slate-400 text-center py-8">No usage data available</p>
        ) : (
